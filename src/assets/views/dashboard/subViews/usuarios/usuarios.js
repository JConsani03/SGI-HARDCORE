...
window.fragmentNamespace = function () {
    ...
    function eliminar() {
        if (selected) {
            fetch(`http://localhost:8080/deleteUsuario/1/${encodeURIComponent(idUsuario)}`)
                .then(res => res.json())
                .then(res => {
                    alert(res.message);
                    clearCampos();
    ...
    function cambiarBoton() {
        if (flag) {
            ...
            selected = false;
            fetch(`http://localhost:8080/createUsuario/1
    ...
    function llenarCampos(user) {
        ...
        document.getElementById('b_editar').disabled = false;
        document.getElementById('b_eliminar').disabled = false;
        fetch(`http://localhost:8080/readUsuario/1/${encodeURIComponent(user)}`)
            .then(res => res.json())
            .then(res => {
    ...
    function llenarTabla(nombre) {
        ...
        fetch(`http://localhost:8080/readUsuario/1/${encodeURIComponent(nombre)}`)
            .then(res => res.json())
            .then(res => {
